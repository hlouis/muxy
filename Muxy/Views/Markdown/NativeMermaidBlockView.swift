...
struct NativeMermaidBlockView: View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 6, style: .continuous)
                    .stroke(Color(nsColor: blend(foreground: palette.foreground, background: palette.background, amount: 0.2)), lineWidth: 1)
            )
    ...
    private func renderFittedDiagramIfNeeded() {
        ...
        let key = "\(trimmedSource.hashValue)-\(Int(targetWidth))-\(palette.background.hexRenderKey)-\(palette.foreground.hexRenderKey)-\(palette.accent.hexRenderKey)"
