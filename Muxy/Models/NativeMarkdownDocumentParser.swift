...
struct NativeMarkdownDocumentParser {
    enum Block: Equatable, Sendable {
        ...
        var markdownText: String {
            ...
            case .mermaid(let source): source
            }
        }
    }
    ...
    static func parse(_ markdown: String) -> [Block] {
        parseSpanned(markdown).map { $0.block }
    }
    ...
    static func parseSpanned(_ markdown: String) -> [SpannedBlock] {
        ...
        func flushMarkdown(upTo index: String.Index, endLine: Int) {
            ...
            if case .markdown(let lastText) = blocks.last?.block {
        ...
        while cursor < end {
            ...
            let lineSansNL: Substring
            if rawLine.hasSuffix("\r\n") {
                lineSansNL = rawLine.dropLast(2)
            } else if rawLine.hasSuffix("\n") || rawLine.hasSuffix("\r") {
                lineSansNL = rawLine.dropLast(1)
            } else {
                lineSansNL = rawLine
            }
            ...
            if let opening = parseOpeningMermaidFence(lineSansNL) {
                ...
                var foundClose: (codeEnd: String.Index, afterClose: String.Index, closeLine: Int)?
                ...
                while searchCursor < end {
                    ...
                    let closeSansNL: Substring
                    if rawCloseLine.hasSuffix("\r\n") {
                        closeSansNL = rawCloseLine.dropLast(2)
                    } else if rawCloseLine.hasSuffix("\n") || rawCloseLine.hasSuffix("\r") {
                        closeSansNL = rawCloseLine.dropLast(1)
                    } else {
                        closeSansNL = rawCloseLine
                    }
